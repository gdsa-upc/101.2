...
def train_classifier(params):
    ...
    for c in set(class_annot):
        if c == 'desconegut':
            #cla = float(total_n_samples / (n_classes * dd))
    ...
    #print lista       
    svr = svm.SVC()        
    parameters = {'kernel':('linear','rbf'), 'C':[1,10,1000,100]}
    a = GridSearchCV(svr,parameters)
    a.fit(feats,class_annot)
    print a.best_params_
    '''         

    sv = svm.SVC(C=412, kernel='rbf', degree=3, gamma='auto', coef0=0.0, shrinking=True, 
