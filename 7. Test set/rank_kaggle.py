...
def rank(params):
    ...
    out = open(os.path.join(params['root'],params['root_save'],params['rankings_dir'],params['descriptor_type'],'test', 'rank.csv'),'w')
    ...
    for test_id in test_features.keys():
        ...
        # Save to text file
        outfile = open(os.path.join(params['root'],params['root_save'],params['rankings_dir'],params['descriptor_type'],'test',test_id.split('.')[0] + '.txt'),'w')
    ...
    for test_id in os.listdir(os.path.join(params['root'],params['root_save'],params['rankings_dir'],params['descriptor_type'],'test')):
        
        #query_class = list(annotation_val.loc[annotation_val['ImageID'] == val_id.split('.')[0]]['ClassID'])[0]     
        rank = pd.read_csv(os.path.join(params['root'],params['root_save'],params['rankings_dir'],params['descriptor_type'],'test',test_id.split('.')[0] + '.txt'),header= None)
        
        #if not query_class == "desconegut":
            
        save_ranking_file(out,test_id,rank)
